...
namespace Savchin.Core
{
    ...
    public static class EnumHelper
    {
        ...
        public static int GetOrder(this Enum enumType)
        {
            var displayAttribute = GetAttribute<DisplayAttribute>(enumType) ?? new DisplayAttribute { Order = 0 };
        ...
        private static T GetAttribute<T>(Enum enumType) where T : class
        {
            var memberInfo = enumType.GetType().GetMember(enumType.ToString());
            ...
            var result = memberInfo[0].GetCustomAttributes(typeof(T), false);
            return result.Length == 0 ? null : result[0] as T;
        }
    }
